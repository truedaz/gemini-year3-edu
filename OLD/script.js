document.addEventListener('DOMContentLoaded', () => {
    ...
    const backToLessonListBtn = document.getElementById('backToLessonListBtn');
    const lessonTitleElem = document.getElementById('lessonTitle');
    const lessonContentElem = document.getElementById('lessonContent');
    ...
    // --- DATA STRUCTURE (Hierarchical: Year Groups > Subjects > Categories > Lessons) ---
    const appData = [
        {
            id: 'year3', title: 'Year 3', icon: 'images/year3_icon.svg', type: 'year_group',
            children: [
                {
                    ...
                    children: [
                        {
                            ...
                            children: [] // Populated by generateTimesTableLesson
                        },
                        {
                            ...
                            learningMaterial: `
                                ...
                                <p>Adding/Subtracting with same denominator: 2/7 + 3/7 = 5/7.</p>
                            `,
                            quiz: [
        ...
        {
            id: 'year4', title: 'Year 4', icon: 'images/year4_icon.svg', type: 'year_group',
            children: [
                {
    ...
    // --- TIMES TABLE GENERATION (ensure unique IDs using prefix) ---
    function generateTimesTableLesson(yearPrefix, num) {
        ...
        answersPool = [...new Set(answersPool)].filter(ans => ans > 0 && ans < 150).sort(() => 0.5 - Math.random()); // Filter out huge/negative numbers
        ...
        for (let i = 0; i < 8; i++) { // Generate 8 quiz questions
            ...
            while(options.length < 4) options.push((parseInt(options[0]) + options.length + Math.ceil(Math.random()*5)).toString()); // Ensure 4 distinct options
    ...
    function loadProgress() {
        const saved = localStorage.getItem('learningAppProgress_v2'); // New key for new structure
        if (saved) {
                ...
                items.forEach(item => {
                    if (item.type === 'lesson') {
                        if (lessonCompletionStatus[item.id] !== undefined) {
                            ...
                            // Also restore quiz answers if saved (more advanced)
                            if (item.quiz && lessonCompletionStatus[`${item.id}_quiz`]) {
                                item.quiz.forEach((q,idx) => {
                                    if(lessonCompletionStatus[`${item.id}_quiz`][idx]){
                                        q.userAnswer = lessonCompletionStatus[`${item.id}_quiz`][idx].userAnswer;
                                        if(q.type === 'drag_and_drop_match') q.userAnswers = lessonCompletionStatus[`${item.id}_quiz`][idx].userAnswers;
                                    }
                                });
                            }
                        }
    ...
    function saveProgress() {
        ...
        localStorage.setItem('learningAppProgress_v2', JSON.stringify(lessonCompletionStatus));
    }

    function countLessonsInItem(item) {
    ...
    function updateOverallProgressDisplay() {
        const overallCounts = countLessonsInItem({ children: appData, type: 'root' });
        const percentage = overallCounts.total > 0 ? (overallCounts.completed / overallCounts.total) * 100 : 0;
        progressBar.style.width = `${percentage}%`;
        progressText.textContent = `${Math.round(percentage)}% Complete (${overallCounts.completed}/${overallCounts.total} lessons)`;
        completedTopicsList.innerHTML = 'Recently Completed: ';
        let listedCount = 0;
        const allLessons = [];
        ...
        collectAllLessons(appData);
        allLessons.filter(l => l.completed).slice(-5).reverse().forEach(l => {
             if(listedCount < 5) {
        ...
        if (listedCount === 0) completedTopicsList.innerHTML += ' None yet!';
        saveProgress();
    }
    ...
    function findItemByPath(pathArray) {
        ...
        let item = null;
        if (!pathArray || pathArray.length === 0) return { children: appData, type: 'root', id: 'root' }; // Return virtual root

        for (const id of pathArray) {
            item = currentLevelItems.find(i => i.id === id);
            if (!item) return null;
            if (item.children) currentLevelItems = item.children;
            else if (pathArray[pathArray.length - 1] !== id) return null; // Path deeper than leaf
            else break;
        }
        return item;
    }
    
    function isHierarchicalItemFullyCompleted(item) {
    ...
    function renderCurrentView() {
        ...
        updateBreadcrumbs();
        addOptionalSubCategoryBackButton();

        let itemsToDisplay;
        const currentContextItem = findItemByPath(currentPath);


        if (!currentContextItem || !currentContextItem.children) { // Should be root or item with children
            console.error("Navigation error. Current context:", currentContextItem, "Path:", currentPath);
            currentPath = []; // Reset to home
            itemsToDisplay = appData;
        } else {
            itemsToDisplay = currentContextItem.children;
        }
        ...
        itemsToDisplay.forEach(item => {
            ...
            const itemCounts = countLessonsInItem(item);
            const itemPercentage = itemCounts.total > 0 ? (itemCounts.completed / itemCounts.total) * 100 : 0;
            const fullyCompleted = isHierarchicalItemFullyCompleted(item);
            ...
            let iconSrc = item.icon || 'images/star_empty.svg';
            if (item.type === 'year_group') { // More specific icon handling
                 iconSrc = item.icon || 'images/year_icon.svg'; // Use assigned or fallback
            }
    ...
    function addOptionalSubCategoryBackButton() {
        let existingButton = navigationHeader.querySelector('#viewSpecificBackButton');
        if (existingButton) existingButton.remove();
        if (currentPath.length > 0) {
            const backButton = document.createElement('button');
            backButton.id = 'viewSpecificBackButton';
            backButton.textContent = '⬅️ Back';
            backButton.title = 'Go to previous category';
            backButton.addEventListener('click', () => {
                currentPath.pop();
                renderCurrentView();
            });
            navigationHeader.insertBefore(backButton, breadcrumbsElem);
        }
    }
    ...
    function navigateToItem(itemId) {
        ...
        if (!parentContext || !parentContext.children) {
             console.error("Cannot navigate: parent context or its children not found for path", currentPath);
             currentPath = []; // Reset to home
             renderCurrentView();
             return;
        }
        ...
        if (item) {
            ...
            if (item.type === 'lesson') showLesson(item);
            else renderCurrentView();
        } else {
            console.error("Item not found for navigation:", itemId, "within context:", parentContext);
        }
    }
    ...
    function showLesson(lesson) {
        ...
        if (lesson.completed) {
            markCompleteBtn.classList.add('hidden');
            if (!quizArea.querySelector('.final-completed-message')) appendLessonCompletedMessage(quizArea);
        } else {
    ...
    function renderQuiz(lesson) {
        ...
        lesson.quiz.forEach((q, index) => {
            if (q.type === 'drag_and_drop_match') {
                renderDragAndDropMatchQuiz(lesson, q, index);
            } else if (q.type === 'radio') {
                renderRadioButtonQuiz(lesson, q, index);
            } else {
    ...
    function renderRadioButtonQuiz(lesson, q, index) {
        ...
        if(!questionDiv){
            ...
            questionDiv.innerHTML = ''; // Clear previous content before re-rendering
        }
        ...
        q.options.forEach(optionText => {
            ...
            input.addEventListener('change', () => {
                ...
                checkSingleRadioAnswer(lesson, q, index, label, feedbackDiv);
            });
        ...
        if (q.userAnswer) {
            ...
            if(selectedLabel) checkSingleRadioAnswer(lesson, q, index, selectedLabel, feedbackDiv);
        }
    }

    function checkSingleRadioAnswer(lesson, questionObject, questionIndex, selectedLabel, feedbackDiv) {
    ...
    function renderDragAndDropMatchQuiz(lesson, q, quizIndex) {
        ...
        q.userAnswers = q.userAnswers || {}; // Initialize if not present

        // Create all draggable items and decide initial placement
        const allDraggableElements = {};
        q.items.forEach(item => {
            allDraggableElements[item.id] = createDraggableElement(item, lesson, q, quizIndex);
        });
        ...
        q.categories.forEach(cat => {
            ...
            dropzone.addEventListener('drop', e => {
                ...
                const originalItemId = draggedElementIdWithPrefix.split('_').slice(1).join('_'); // Get original item id
                const draggedElement = document.getElementById(draggedElementIdWithPrefix);

                if (draggedElement) {
                    // If item was in another dropzone, clear its old answer
                    if (q.userAnswers[originalItemId] && q.userAnswers[originalItemId] !== cat.id) {
                        // No need to delete, just overwrite
                    }
                    // If item was in the pool, it's now being placed
        ...
        // Place items based on q.userAnswers or in pool
        q.items.forEach(item => {
            ...
            if (q.userAnswers[item.id]) {
                ...
                else draggablesPool.appendChild(draggableElement); // Fallback to pool
            } else {
                draggablesPool.appendChild(draggableElement);
            }
        });

        // Allow dragging from dropzone back to pool
    ...
    function createDraggableElement(item, lesson, q, quizIndex) {
    ...
    function checkAllQuizAnswers(lesson) {
        ...
        lesson.quiz.forEach((q, index) => {
            ...
            if (q.type === 'drag_and_drop_match') {
                ...
                // Feedback is handled by checkSingleRadioAnswer, but this check determines overall completion
            }
            if (!currentQuestionCorrect) allQuestionsCorrect = false;
        });
        if (allQuestionsCorrect && !lesson.completed) {
            markCompleteBtn.classList.remove('hidden');
    ...
    markCompleteBtn.addEventListener('click', () => {
        if (currentLesson && !currentLesson.completed) {
            ...
            updateOverallProgressDisplay(); // This calls saveProgress
            markCompleteBtn.classList.add('hidden');
